(function (exports, shared) {
  ...
  const parseElement = (context) => {
      ...
      // 但是 <br /> 合法，<br> 也是合法的
      // 因此用 isVoidTag 判断一下
      if (element.isSelfClosing || shared.isVoidTag(element.tag)) {
          return element;
      }
  ...
  const parseTag = (context) => {
      ...
      // 判断是组件还是原生元素
      const tagType = shared.isHTMLTag(tag)
  ...
  */
      ...
      return function postTransformElement() {
          ...
          if (patchFlag !== 0) {
              ...
              // patchFlag 为正数说明可能存在复合情况，特殊处理
                  ...
                  Object.keys(shared.PatchFlagNames)
                      // 全部转换为 Number 类型
                      .map(Number)
  ...
  const transformOn = (dir) => {
      ...
      if (arg.type === 3 /* NodeTypes.SIMPLE_EXPRESSION */) {
          if (arg.isStatic) {
              ...
              eventName = createSimpleExpression(shared.toHandlerKey(shared.capitalize(rawName)), //capitalize(rawName)：把事件名首字母大写;  toHandlerKey("Click")：添加 on 前缀;  createSimpleExpression("onClick", true)：包装成一个静态表达式节点。  
              true);
          }
  ...
  const genChildren = (children, context) => {
      ...
      // 单独处理 TEXT
      else if (shared.isObject(children) && children.type === 2 /* NodeTypes.TEXT */) {
  ...
  const baseCompile = (template, options = {}) => {
      // 第一步：将模板字符串转换成AST
      const ast = shared.isString(template) ? baseParse(template) : template;
      // 第二步：AST加工
...
})(this.VueCompilerCore = this.VueCompilerCore || {}, VueShared);
